    ...
    for block in hps:
        ...
        for line in block.get("hl") or []:
            ...
            for ol in line.get("ol") or []:
                ...
                on = str(ol.get("on") or "")
                key = None
                if ot == "over" or on.startswith("大"):
                    key = "over"
                elif ot == "under" or on.startswith("小"):
                    key = "under"
                if not key:
                    continue
                odds_data[key] = price
...
def _half_totals_from_hps(
    ...
    for block in hps:
        ...
        # 识别半场：hpn 含"上半场"或"下半场"且含"大小"
        if "上半场" in hpn and "大小" in hpn:
            half_bt = "first_half_total"
            half_label = "上半场"
        elif "下半场" in hpn and "大小" in hpn:
            half_bt = "second_half_total"
            half_label = "下半场"
        else:
            continue
        for line in block.get("hl") or []:
            ...
            if "under" in odds_data:
                ...
                out.append(RemoteOdds(bet_type=half_bt, odds_data=odds_data, total=total))
    return out
...
def parse_matches_pb(rows: list, *, limit: int = 800) -> list[RemoteMatch]:
    ...
    for row in rows:
        ...
        # 上下半场大小球
        half_totals = _half_totals_from_hps(hps, mid=mid, csid=csid, tid=tid, match_type=match_type)
        odds_list.extend(half_totals)
        if not odds_list:
            continue
        seen.add(mid)
        ...
        # 完场/取消不再当滚球
        mmp = str(row.get("mmp") or "").strip()
        if period == "完场" or mmp in ("8", "100", "999", "12"):
            status = "finished"
            clock = ""
        out.append(
            RemoteMatch(
...
async def fetch_ybty_matches_odds(
    *,
    ...
    def _sport_ctx_from_url(u: str) -> tuple[str, str, str]:
        ...
        if not any(
            x in low
            for x in ("yewu", "app-h5", "zlshelves", "token=", "/#/match", "sportstype")
        ):
    ...
    async def _fetch_matches_pb(page, sport_token: str, cuid: str) -> list:
        payloads = await page.evaluate(
            """async ({ sportToken, cuid, specs }) => {
              const headers = {
              ...
              for (const spec of specs) {
                try {
                  const resp = await fetch(
                    'https://api.rccg5fz.com/yewu11/v1/m/matchesPB?t=' + Date.now(),
                    {
                      method: 'POST',
                      headers,
                      body: JSON.stringify({
                  ...
                  outs.push({ error: String(e) });
                }
              }
              return outs;
            }""",
            {"sportToken": sport_token, "cuid": cuid, "specs": query_specs},
        )
    ...
    async def _run_on_page(page) -> list:
        ...
        try:
            for u in await _collect_page_urls(page):
                hu, st, cuid = _sport_ctx_from_url(u)
                if st:
                    h5_url = hu
                    logger.info("OB odds: reuse current venue url (no navigate)")
                    try:
                        await dismiss_h5_orient_tip(page)
                    except Exception:
                        pass
                    return await _fetch_matches_pb(page, st, cuid)
        ...
        if not own_browser:
                    ...
                    for u in await _collect_page_urls(page):
                        ...
                        if st:
                            ...
                            try:
                                ...
                                pass
                            logger.info("OB odds: already in venue — no reload/goto")
                            return await _fetch_matches_pb(page, st, cuid)
            except Exception:
                pass
                ...
                if st:
                    try:
                        ...
                        try:
                            ...
                            pass
                        for u in await _collect_page_urls(page):
                            hu2, st2, cuid2 = _sport_ctx_from_url(u)
                            if st2:
                                h5_url = hu2
                                restored = True
                                return await _fetch_matches_pb(page, st2, cuid2)
            ...
            logger.warning(
                "OB venue page has no sport token after restore/re-enter; url=%s",
                (getattr(page, "url", "") or "")[:160],
            )
            return []
        ...
        try:
            ...
            if not sport_token:
                return []
            return await _fetch_matches_pb(page, sport_token, cuid)
        except Exception:
