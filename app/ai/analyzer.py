...
class MatchAnalyzer:
    ...
    async def _call_deepseek(self, prompt: str, system_extra: str = "") -> dict:
        ...
        _system_content = (
            ...
            "4. 反向风险：under高置信度保持保守；over只有盘口/节奏/基本面三项同向时才可达高概率\n"
    ...
    def _synchronize_direction_confidences(analysis: dict) -> dict:
        ...
        方向概率、实际校准概率不一致时仍进入后续闸门。现在预测方向严格等于
...
#### 置信度
- 5-6/6维度+强一致->0.60-0.82（≥0.78必须盘口/节奏/基本面三项同向）
...
#### 置信度
- 5-6/6维度+信号一致->0.62-0.82（≥0.78必须盘口/节奏/基本面三项同向）
...
"""
        ...
        if sport == "basketball":
            prompt += (
                ...
                "4) 低分走势占优\n"
                "盘口强信号（降盘≥5分）+任一其他同向证据 -> 可给 conf 0.58-0.72；"
                "盘口/节奏/基本面三项强同向时才可达0.78以上\n"
                ...
                "- 三类信号全矛盾 -> 必须 skip\n"
                "- NBA/ACB/欧篮联under自动门槛0.58，其他合规篮球under门槛0.62；只有多维度一致才可达到\n"
            ...
            prompt += (
                ...
                "- 重点/合规超级甲级联赛under自动门槛0.58，其他职业赛事0.62；只有强一致证据才可达到\n"
                "- confidence 必须与信号强度匹配，不得虚高\n"
            )
        ...
        if sport == "basketball":
            prompt += (
                ...
                "4) 高分走势占优\n"
                "盘口强信号（资金推动型升盘≥5分+水位下降）+任一其他同向证据 -> 可给 conf 0.58-0.72；"
                "盘口/节奏/基本面三项强同向时才可达0.78以上\n"
                ...
                "- 三类信号全矛盾 -> 必须 skip\n"
                "- NBA/ACB/欧篮联over自动门槛0.68，其他合规篮球over门槛0.78；只有多维度强一致才可达到\n"
                "- confidence 必须与信号强度匹配，不得虚高\n"
            )
        else:
            prompt += (
                ...
                "- 三类信号（初指/实时盘口/基本面）全矛盾 -> 必须 skip\n"
                "- 重点/合规超级甲级联赛over自动门槛0.68，其他职业赛事0.78\n"
                "- confidence 必须与信号强度匹配，不得虚高\n"
    ...
    def _limit_analysis_prompt(
        ...
        *,
        ...
        snapshot = {
            ...
            "total_line": line,
            "auto_thresholds": {
                "under": balanced_min_confidence(sport, "under", league),
                "over": balanced_min_confidence(sport, "over", league),
            },
        }
        mandatory = (
            "## 压缩后权威快照（不得改写）\n"
