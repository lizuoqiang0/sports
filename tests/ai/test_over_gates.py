...
async def run_cases():
    ...
    # ── 2. A1 影子模式：AI_ENABLE_OVER=False 时 over 被拦 ──
    eng = _mk_engine(over_enabled=False)
    d = await _eval(eng, _base_match(), _analysis())
    results.append(("影子模式 over 被拒（未启用）", not d.should_bet and "未启用" in d.reasoning,
    ...
    # ── 4. A3 方向隔离地板：over 0.62 / under 0.55 ──
    fb = SPORT_RISK["football"]
    results.append(("over 地板 0.62 > under 0.55",
                    fb["over_min_conf"] == 0.62 and fb["under_min_conf"] == 0.55,
                    f"over={fb['over_min_conf']} under={fb['under_min_conf']}"))
    # conf=0.58：过 under 地板但不过 over 地板（无基本面）
    eng = _mk_engine(over_enabled=True)
    d = await _eval(eng, _base_match(), _analysis(confidence=0.58, context_source="none"))
    results.append(("conf=0.58 过 under 不过 over 地板", not d.should_bet and "置信度不足" in d.reasoning,
                    f"reasoning={d.reasoning}"))
    ...
    # ── 6. B3 高赔率镜像 ──
    eng = _mk_engine(over_enabled=True)
    d = await _eval(eng, _base_match(odds={"under": 1.90, "over": 2.10}), _analysis(odds=2.10, line=2.5))
    results.append(("over 赔率≥2.0 拒（市场看小）", not d.should_bet and "大球赔率过高" in d.reasoning,
                    f"reasoning={d.reasoning}"))

    # ── 7. C1 升盘→over 支持：升盘+over 通过方向检查（降盘+over 拒）──
    eng = _mk_engine(over_enabled=True)
    d = await _eval(eng, _base_match(line_movements={"total": {"line_delta": -0.5}}),
                    _analysis(line=2.5))
    ...
    d = await _eval(eng, _base_match(), _analysis(confidence=0.65))
    # over 样本 2<5：不因 under 烂而加严（0.65 过 0.62 地板）
    ...
    # ── 12. 仓位：over 无 1.10 加成且 0.6 折 ──
    # 双闸门互斥（同场景难同时过 D1），直接验证仓位公式：
    # under: conf_scale×1.10 封顶 0.95；over: conf_scale 封顶后 ×0.6
    eng = _mk_engine(over_enabled=True)
    d = await _eval(eng, _base_match(home_score=2, away_score=1, clock="", line=2.5),
                    _analysis(confidence=0.70, line=2.5))
    # over 场景 conf=0.70≥0.65 → conf_scale=0.90 → ×0.6=0.54 → 仓位=100×0.54×risk_factor
    # under 公式（历史行为）: 0.90×1.10 封顶 0.95 → 仓位=100×0.95×risk_factor
    # 验证 over 仓位 ≈ 0.54/0.95 ≈ 57% 的 under 等效仓位
    results.append(("over 仓位 0.6 折生效（≈54/100 上限）",
                    d.should_bet and 0 < d.suggested_stake <= 54,
                    f"over_stake={d.suggested_stake}(bet={d.should_bet}) 期望≤54"))
