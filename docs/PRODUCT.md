...
| 自动投注 | 每 30 分钟一轮，最多 2 场不同比赛，Kelly 仓位计算 |
...
| 模式 | 文案 |
|------|------|
| 自动 | 自动投注 - 每 30 分钟一轮，最多 2 场不同比赛真实下单 |
...
```
每 30 分钟一轮 (AI_SCAN_INTERVAL_SEC=1800):
  ├─ 1. 风控检查
  │   ├─ 计算每日盈亏（当前总资产 - 午夜基线）
  ...
  │   └─ 仓位 ∈ [1, max_bet_amount]
  │
  ├─ 6. 最多选 2 场不同比赛 (AI_MAX_BETS_PER_CYCLE=2)
  │   OB/平博同场只允许一单
  │
  ├─ 7. 真实下单
  │   ├─ Redis 分布式锁防双发
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `AI_SCAN_INTERVAL_SEC` | 1800 | 扫描间隔（秒），范围 [60, ∞) |
