...
## 八、性能优化（2026-08-20 第二批：分析/投注速度）

### 8.1 瓶颈分析（原链路）

```
扫描→分析→下单全链路三个速度瓶颈：
1. 固定 120s 轮询间隔：无候选时也死等 120s，错过刚开赛/滚球上新的黄金分析窗口
2. LLM 分析并发 3：8 个同场分组需 ~3 波串行（每波 8-40s），轮周期被拉长
3. 同场重复 LLM 调用：LLM 判 skip 后，下一轮（120s 后）对同场再发起 LLM 调用
   （缓存 TTL 600s 内比赛状态无质变，重复判定结果几乎相同，纯浪费）
```

### 8.2 优化实现（app/ai/auto_better.py）

| 优化项 | 变更前 | 变更后 | 效果 |
|--------|--------|--------|------|
| 动态轮询间隔 | 固定 max(120, AI_SCAN_INTERVAL_SEC) | 有候选=AI_SCAN_INTERVAL_SEC（≥60）；**空轮=_IDLE_RESCAN_SEC（默认 30s）** | 新滚球进入分析视野的最坏延迟 120s→30s |
| LLM 分析并发 | AI_ANALYZE_CONCURRENCY 默认 3 | 默认 **8**（与 recs_job 对齐） | 8 组从 ~3 波 → 1 波，轮内分析墙钟时间 ↓ ~66% |
| 同场 skip 冷却 | 每轮全量重分析 | LLM 判 skip 后 _SKIP_COOLDOWN_SEC（默认 300s）内跳过该场 LLM 调用 | 单轮 LLM 调用数显著下降（skip 场次占多数），省 token 省时延 |
| 扫描层短路（已有） | — | 无盘口/赔率区间外的场次不进 LLM（E1 前置） | 无效 LLM 调用为 0 |

### 8.3 新增配置项

```python
AI_ANALYZE_CONCURRENCY = 8    # LLM 同轮并发（原 3）
AI_IDLE_RESCAN_SEC = 30       # 空轮快扫间隔（原死等 120s）
AI_SKIP_COOLDOWN_SEC = 300    # 同场 skip 冷却（原无，每轮重分析）
# AI_SCAN_INTERVAL_SEC = 120  # 有候选时轮间隔（不变，≥60 下限）
```

### 8.4 冷却实现细节

- 冷却键：同场分组内最小 match_id（多站同场稳定映射）
- 记录时机：LLM 返回 selection=skip（或 over 影子模式跳过）时
- 失效清理：每轮开始时顺手清理过期项（dict 无界增长防护）
- 保守语义：**只冷却 skip，不冷却闸门拒绝**——闸门拒绝（如 D1 余量不足）
  可能随比赛进行被补足，需每轮重评；skip 是"信号中性"判定，短期翻转概率低

### 8.5 预期收益（8 场滚球典型场景）

```
原链路：空轮等待 120s + 分析 3 波 × ~15s = 最坏 165s 才触达新场次
新链路：空轮等待 30s  + 分析 1 波 × ~15s = 最坏 45s 触达新场次
LLM 调用/轮：8 场（全量）→ 首轮 8 场，后续轮 skip 场次冷却 = 仅增量场次
```
...
| tests/ai/test_speed_optimization.py | **性能优化 10 项**（动态间隔/并发数/skip冷却生命周期/返回值契约） | 10/10 |
| tests/ 全量 | 回归保护 | 150/150 |
...
4. **skip 冷却的边界**：冷却期内若比赛状态质变（红牌/大比分），需等冷却结束才重新分析——以 300s 窗口换取调用效率，属可接受折衷
